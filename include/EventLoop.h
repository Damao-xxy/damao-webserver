...
public:
     ...
     * 定时任务相关函数
     */
    void runAt(Timestamp timestamp, Function &&cb)
    {
        timerQueue_->addTimer(std::move(cb), timestamp, 0.0);
    }

    void runAfter(double waitTime, Function &&cb)
    {
        Timestamp time(addTime(Timestamp::now(), waitTime));
        runAt(time, std::move(cb));
    }

    void runEvery(double interval, Function &&cb)
    {
...
private:
    ...
    void doPendingFunctions(); // 执行上层回调
